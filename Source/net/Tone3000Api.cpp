...
bool looksLikeA2(const std::string& s) {
    ...
    return std::tolower(static_cast<unsigned char>(s[0])) == 'a' && s[1] == '2';
}
...
std::string buildAuthorizeUrl(const std::string& publishableKey, const std::string& redirectUri,
    ...
    if (!prompt.empty()) url += "&prompt=" + urlEncode(prompt);
    return url;
}
...
std::string modelsUrl(const std::string& toneId, const std::string& architecture) {
    ...
    if (!architecture.empty()) url += "&architecture=" + urlEncode(architecture);
...
TokenResponse parseTokenResponse(const std::string& json) {
    ...
    try {
        ...
        if (!j.contains("access_token")) return result;
...
std::string buildSearchUrl(const std::string& query, int page, int pageSize, bool namOnly) {
    ...
    if (!query.empty()) url += "query=" + urlEncode(query) + "&";
    url += "page=" + std::to_string(page);
    url += "&page_size=" + std::to_string(pageSize);
    if (namOnly) url += "&format=nam";
    return url;
}

std::string buildSearchUrl(const SearchParams& p) {
    std::string url = std::string(kBaseUrl) + "/tones/search?";
    if (!p.query.empty()) url += "query=" + urlEncode(p.query) + "&";
    ...
    if (p.architecture > 0) url += "&architecture=" + std::to_string(p.architecture);
    return url;
}

std::string buildTrendingUrl() { return std::string(kBaseUrl) + "/tones/trending"; }

std::vector<ToneInfo> parseToneList(const std::string& json) {
    std::vector<ToneInfo> tones;
    try {
        auto j = nlohmann::json::parse(json);
        if (!j.contains("data") || !j["data"].is_array()) return tones;
        for (const auto& entry : j["data"]) {
            // Each entry is parsed independently: a single malformed/odd
            // entry must not drop the rest of an otherwise-usable list.
            try {
