...
void ToneEq::recalcIfChanged() {
    ...
    if (lo != lastLow_) {
        const float z1 = low_.z1, z2 = low_.z2;
        low_ = Biquad::lowShelf(sr_, 100.0f, lo);
        low_.z1 = z1;
        low_.z2 = z2;
        lastLow_ = lo;
    }
    if (md != lastMid_) {
        const float z1 = mid_.z1, z2 = mid_.z2;
        mid_ = Biquad::peaking(sr_, 700.0f, 0.7f, md);
        mid_.z1 = z1;
        mid_.z2 = z2;
        lastMid_ = md;
    }
    if (hi != lastHigh_) {
        const float z1 = high_.z1, z2 = high_.z2;
        high_ = Biquad::highShelf(sr_, 3200.0f, hi);
        high_.z1 = z1;
        high_.z2 = z2;
        lastHigh_ = hi;
    }
