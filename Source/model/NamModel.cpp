...
std::unique_ptr<NamModel> NamModel::load(const std::string& path, int sampleRate, int maxBlock) {
    ...
    try {
        ...
        return m;
    } catch (const std::exception&) { return nullptr; } catch (...) {
        return nullptr;
    }
}

NamModel::~NamModel() { delete static_cast<NeuralModel*>(model_); }
...
void NamModel::process(const float* input, float* output, int numSamples) {
    ...
    static_cast<NeuralModel*>(model_)->Process(const_cast<float*>(input), output,
