...
void LibraryPanel::EntryListModel::paintListBoxItem(int rowNumber, juce::Graphics& g, int width,
    ...
    if (type_ == nam::LibraryType::Model) {
        ...
        if (e.sampleRate > 0)
            hint += (hint.isEmpty() ? juce::String() : juce::String(" @ ")) +
                    juce::String(e.sampleRate) + " Hz";
    }

    auto textArea = juce::Rectangle<int>(kStarWidth + 4, 0, width - kStarWidth - 8, height);

    // Reserve a right-hand slice for the hint so a long name can't overlap it.
    const int hintWidth = hint.isNotEmpty() ? (textArea.getWidth() * 42 / 100) : 0;
...
LibraryPanel::LibraryPanel(nam::LibraryStore& store)
    : store_(store), modelsModel_(store, nam::LibraryType::Model, *this),
...
std::vector<nam::LibraryEntry>
LibraryPanel::sortedFavoritesFirst(std::vector<nam::LibraryEntry> entries) {
    ...
    std::stable_sort(
        entries.begin(), entries.end(), [](const nam::LibraryEntry& a, const nam::LibraryEntry& b) {
            if (a.favorite != b.favorite) return a.favorite;
            if (a.lastUsedAt != b.lastUsedAt) return a.lastUsedAt > b.lastUsedAt;
            return juce::String(a.displayName).compareIgnoreCase(juce::String(b.displayName)) < 0;
        });
    return entries;
}
