...
void LiveScreen::paint (juce::Graphics& g) {
    ...
    text ("Setlist", displayFont (28.0f), col::ink, { 20, 34, 240, 40 }, juce::Justification::topLeft);
    ...
    for (int i = 0; i < (int) rowRects_.size(); ++i) {
        ...
        text (juce::String (i + 1), displayFont (34.0f),
              on ? col::accentAlt : col::inkA (0.35f),
        ...
        text (juce::String (slots_[(size_t) i].displayName), uiFont (20.0f, true),
              on ? col::ink : col::inkA (0.6f),
              inner.withTrimmedRight (24), juce::Justification::centredLeft);
    }
    ...
    text ("3.2 ms", uiFont (12.0f, true), col::inkA (0.55f),
          getLocalBounds().removeFromBottom (52).withTrimmedRight (20), juce::Justification::centredRight);
}

void LiveScreen::mouseDown (const juce::MouseEvent& e) {
    const auto p = e.getPosition();
    if (exitRect_.expanded (8).contains (p)) { if (onExit) onExit(); return; }
    for (int i = 0; i < (int) rowRects_.size(); ++i)
