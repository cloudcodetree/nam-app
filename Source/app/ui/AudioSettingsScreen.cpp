...
juce::String deviceMeta (const juce::String& name, bool input) {
    if (name.containsIgnoreCase ("usb") || name.containsIgnoreCase ("irig")) return "USB audio";
    if (name.containsIgnoreCase ("default")) return input ? "system routing" : "system routing";
    ...
    if (name.containsIgnoreCase ("bluetooth")) return "Bluetooth";
    return input ? "input" : "output";
}
}   // namespace

AudioSettingsScreen::AudioSettingsScreen () {
    setOpaque (true);
    startTimerHz (30);
}
...
void AudioSettingsScreen::timerCallback () {
    if (gain_ == Gain::Listening && ++gainTicks_ > 120) {   // ~4 s window
        ...
        appliedDb_ = 0.0f;
        if (gainMax_ > 0.02f)
            appliedDb_ = juce::jlimit (-24.0f, 24.0f, 20.0f * std::log10 (0.25f / gainMax_));
    ...
    for (auto r : { meterInRow_, meterOutRow_, gainBtn_ }) {
        const auto vis = r.translated (0, dy).expanded (4).getIntersection (
            getLocalBounds ().withTrimmedTop (contentTop_));
...
void AudioSettingsScreen::relayout () {
    ...
    top = beginCard ();
    y += pad;
    outHeader_ = { x + pad, y, w - pad * 2, headH };
    y += headH;
    outRects_.clear ();
    ...
    top = beginCard ();
    y += pad;
    engineHeader_ = { x + pad, y, w - pad * 2, headH };
    y += headH;
    ...
    y += chipH + 12;
    latencyRow_ = { x + pad, y, w - pad * 2, 26 };
    y += 26;
    endCard (top);
    ...
    top = beginCard ();
    y += pad;
    levelsHeader_ = { x + pad, y, w - pad * 2, headH };
    y += headH;
    meterInRow_ = { x + pad, y, w - pad * 2, 24 };
    y += 28;
    meterOutRow_ = { x + pad, y, w - pad * 2, 24 };
    y += 32;
    gainBtn_ = { x + pad, y, w - pad * 2, 42 };
    y += 42;
    endCard (top);
...
void AudioSettingsScreen::paint (juce::Graphics& g) {
    ...
    auto deviceRow = [&] (juce::Rectangle<int> rr, const juce::String& name, bool selected,
                          bool input) {
        ...
        if (selected)
            text (kCheck, uiFont (13.0f, false), col::accentAlt, tx.removeFromRight (20),
        ...
        const bool routed =
            !input && name.containsIgnoreCase ("default") && st_.outputRouteHint.isNotEmpty ();
    ...
    auto chip = [&] (juce::Rectangle<int> rr, const juce::String& label, bool on) {
        ...
        text (label, uiFont (12.0f, true), on ? col::accentAlt : col::inkA (0.6f), S (rr),
              juce::Justification::centred);
    };
    auto meter = [&] (juce::Rectangle<int> rr, const juce::String& label, float peak,
                      bool tickTarget, bool orangeTail) {
        auto row = S (rr);
        text (label, uiFontTracked (10.0f, true), col::inkA (0.4f), row.removeFromLeft (32),
              juce::Justification::centredLeft);
        ...
        juce::ColourGradient mg (col::meterGreen, bar.getX (), 0,
                                 orangeTail ? col::accent : col::meterLime, bar.getX () + w, 0,
                                 false);
        if (orangeTail) mg.addColour (0.7, col::meterLime);
        g.setGradientFill (mg);
        juce::Path p;
        p.addRoundedRectangle (bar.withWidth (w), 4.0f);
        g.fillPath (p);
        if (tickTarget) {
            g.setColour (col::inkA (0.5f));
            g.fillRect (bar.getX () + bar.getWidth () * 0.82f, bar.getY () - 2.0f, 2.0f,
                        bar.getHeight () + 4.0f);
        }
    };
    ...
    for (int i = 0; i < st_.outputs.size () && i < (int)outRects_.size (); ++i)
        deviceRow (outRects_[(size_t)i], st_.outputs[i], st_.outputs[i] == st_.currentOutput,
                   false);

    // ENGINE card
    sectionLabel (engineHeader_, "ENGINE");
    text ("sample rate", uiFont (13.0f, false), col::inkA (0.6f), S (rateRow_).removeFromLeft (80),
          juce::Justification::centredLeft);
    for (int i = 0; i < st_.rates.size () && i < (int)rateRects_.size (); ++i)
        chip (rateRects_[(size_t)i], st_.rates[i], st_.rates[i] == st_.currentRate);
    text ("buffer", uiFont (13.0f, false), col::inkA (0.6f), S (bufRow_).removeFromLeft (80),
          juce::Justification::centredLeft);
    ...
    meter (meterOutRow_, "OUT", outPeak_, false, true);
    {
        ...
        if (gain_ == Gain::Listening)
            label = "listening" + juce::String::fromUTF8 ("\xE2\x80\xA6") + " play a few chords";
        if (gain_ == Gain::Applied)
            label = "input set " + kDot + " " + juce::String (appliedDb_, 1) + " dB";
        text (label, uiFontTracked (12.0f, true),
              listening || gain_ == Gain::Applied ? col::accentAlt : col::inkA (0.7f), S (gainBtn_),
              juce::Justification::centred);
    }
...
void AudioSettingsScreen::mouseUp (const juce::MouseEvent& e) {
    ...
    if (rescanRect_.contains (cp)) {
        if (onRescan) onRescan ();
        return;
    }
    if (gainBtn_.contains (cp)) {
        if (gain_ == Gain::Idle) {
            gain_ = Gain::Listening;
            gainTicks_ = 0;
            gainMax_ = 0.0f;
        }
        return;
    ...
    for (int i = 0; i < (int)inRects_.size () && i < st_.inputs.size (); ++i)
        if (inRects_[(size_t)i].contains (cp)) {
            if (onSelectInput) onSelectInput (st_.inputs[i]);
            return;
        }
    for (int i = 0; i < (int)outRects_.size () && i < st_.outputs.size (); ++i)
        if (outRects_[(size_t)i].contains (cp)) {
            if (onSelectOutput) onSelectOutput (st_.outputs[i]);
            return;
        }
    for (int i = 0; i < (int)rateRects_.size () && i < st_.rates.size (); ++i)
        if (rateRects_[(size_t)i].contains (cp)) {
            if (onSelectRate) onSelectRate (st_.rates[i]);
            return;
        }
    for (int i = 0; i < (int)bufRects_.size () && i < st_.buffers.size (); ++i)
        if (bufRects_[(size_t)i].contains (cp)) {
            if (onSelectBuffer) onSelectBuffer (st_.buffers[i]);
            return;
        }
}

void AudioSettingsScreen::mouseWheelMove (const juce::MouseEvent&,
