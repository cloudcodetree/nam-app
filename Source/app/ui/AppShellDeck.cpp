...
juce::Image thumbOf (std::map<std::string, juce::Image>& cache, const std::string& key,
                     const juce::Image& full) {
    if (auto it = cache.find (key); it != cache.end () && it->second.isValid ()) return it->second;
    if (!full.isValid ()) return {};
    constexpr int side = 160;
    const float s =
        (float)side / (float)juce::jmax (1, juce::jmax (full.getWidth (), full.getHeight ()));
    ...
    if (deckMode_ == 2) {
        ...
        for (const auto& t : playDeck_) {
            ...
            it.thumb = thumbOf (thumbCache_, "t_" + t.id,
                                svc_.artworkForTone ? svc_.artworkForTone (t) : juce::Image ());
        ...
        for (const auto& e : favDeck ()) {
            ...
            if (e.type == nam::LibraryType::Ir) {
                ...
                it.sub = (a.contains ("slim") || a.startsWith ("2") || a.startsWith ("a2")) ? "A2"
                                                                                            : "A1";
            } else {
                it.sub = "MY TONES";
            }
            it.kept = e.favorite;
            it.thumb = thumbOf (thumbCache_, "e_" + e.id, artwork_ ? artwork_ (e) : juce::Image ());
            items.push_back (std::move (it));
        }
    }
