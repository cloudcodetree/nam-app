...
void LibraryScreen::paint (juce::Graphics& g) {
    ...
    text ("Library", displayFont (30.0f), col::ink, { 44, 20, 200, 44 }, juce::Justification::centredLeft);
    text (juce::String ((int) all_.size()) + " tones kept", uiFont (12.0f, false), col::inkA (0.45f),
          { getWidth() - 160, 28, 140, 28 }, juce::Justification::centredRight);
    ...
    for (int i = 0; i < (int) cardRects_.size(); ++i) {
        ...
        // art placeholder
        juce::Path clip; clip.addRoundedRectangle (art.toFloat(), 12.0f);
        ...
        g.setGradientFill (ag); g.fillRect (art);
        text (juce::String (e.displayName).substring (0, 1).toUpperCase(),
              displayFont (art.getHeight() * 0.5f), col::inkA (0.12f), art, juce::Justification::centred);
        g.restoreState();
...
void LibraryScreen::mouseDown (const juce::MouseEvent& e) {
    ...
    for (int i = 0; i < (int) filterRects_.size(); ++i)
        if (filterRects_[(size_t) i].contains (p)) { filter_ = i; applyFilter(); relayout(); repaint(); return; }
    for (int i = 0; i < (int) cardRects_.size() && i < (int) shown_.size(); ++i)
        if (cardRects_[(size_t) i].contains (p)) { if (onLoad) onLoad (shown_[(size_t) i]); return; }
}
