...
EditScreen::EditScreen (dsp::ToneEngine& engine) : engine_ (engine) {
    ...
    // --- GATE ---
    { Stage s; s.key = "GATE";
      s.faders.push_back (makeFader ("thresh", 28.0,
          [] (double v) { return kMinus + juce::String ((int) std::lround (70 - v * 0.5)) + " dB"; },
          [this] (double v) { engine_.setGateThresholdDb ((float) -(70.0 - v * 0.5)); }));
      s.summary = [this] { return kMinus
          + juce::String ((int) std::lround (70 - stages_[0].faders[0].slider->getValue() * 0.5)) + " dB"; };
      stages_.push_back (std::move (s)); }

    // --- AMP (open by default) ---
    { Stage s; s.key = "AMP"; s.open = true;
      s.faders.push_back (makeFader ("gain", 70.0, [] (double v) { return juce::String ((int) v) + "%"; },
          [this] (double v) { engine_.setInputDb ((float) (-24.0 + v * 0.48)); }));
      s.faders.push_back (makeFader ("level", 50.0, [] (double v) { return juce::String ((int) v) + "%"; },
          [this] (double v) { engine_.setOutputDb ((float) (-24.0 + v * 0.48)); }));
      s.summary = [this] { return "gain " + juce::String ((int) stages_[1].faders[0].slider->getValue())
    ...
    { Stage s; s.key = "CAB"; s.lockable = true;
      s.faders.push_back (makeFader ("low cut", 20.0,
          [] (double v) { return juce::String ((int) std::lround (40 + v * 2.6)) + "Hz"; }, [] (double) {}));
      s.faders.push_back (makeFader ("hi cut", 75.0,
          [] (double v) { return juce::String (6 + (int) std::lround (v * 0.06)) + "kHz"; }, [] (double) {}));
      s.summary = [] { return "Greenback 4" + kMult + "12"; };
      stages_.push_back (std::move (s)); }

    // --- EQ ---
    { Stage s; s.key = "EQ";
      s.faders.push_back (makeFader ("low", 50.0, signedDb,
          [this] (double v) { engine_.setLowDb ((float) ((v - 50.0) / 8.0)); }));
      s.faders.push_back (makeFader ("mid", 44.0, signedDb,
          [this] (double v) { engine_.setMidDb ((float) ((v - 50.0) / 8.0)); }));
      s.faders.push_back (makeFader ("high", 56.0, signedDb,
          [this] (double v) { engine_.setHighDb ((float) ((v - 50.0) / 8.0)); }));
      s.summary = [this] { auto& f = stages_[3].faders;
    ...
    // --- DELAY ---
    { Stage s; s.key = "DELAY";
      s.faders.push_back (makeFader ("time", 0.0,
          [] (double v) { return v == 0 ? juce::String ("off") : juce::String ((int) std::lround (80 + v * 6)) + "ms"; },
          [this] (double v) { engine_.setDelayTimeMs ((float) (80.0 + v * 6.0)); engine_.setDelayEnabled (v > 0); }));
      s.faders.push_back (makeFader ("mix", 0.0, [] (double v) { return juce::String ((int) v) + "%"; },
          [this] (double v) { engine_.setDelayMix ((float) (v / 100.0)); }));
      s.summary = [this] { auto& f = stages_[4].faders;
          return f[0].slider->getValue() == 0 ? juce::String ("off")
              : juce::String ((int) std::lround (80 + f[0].slider->getValue() * 6)) + "ms " + kDot + " "
                + juce::String ((int) f[1].slider->getValue()) + "%"; };
      stages_.push_back (std::move (s)); }

    // --- REVERB ---
    { Stage s; s.key = "REVERB";
      s.faders.push_back (makeFader ("size", 25.0,
          [] (double v) { return v < 33 ? juce::String ("room") : v < 66 ? juce::String ("hall") : juce::String ("plate"); },
          [this] (double v) { engine_.setReverbRoomSize ((float) (v / 100.0)); }));
      s.faders.push_back (makeFader ("mix", 20.0, [] (double v) { return juce::String ((int) v) + "%"; },
          [this] (double v) { engine_.setReverbMix ((float) (v / 100.0)); engine_.setReverbEnabled (v > 0); }));
      s.summary = [this] { auto& f = stages_[5].faders;
          const double sz = f[0].slider->getValue();
          return juce::String (sz < 33 ? "room" : sz < 66 ? "hall" : "plate")
...
void EditScreen::relayout() {
    ...
    for (auto& s : stages_) {
        ...
        if (s.open) {
            ...
            if (s.lockable) { lockRect_ = { list_.getRight() - 12 - 108, fy + 4, 108, 32 }; fy += 44; }
            y = fy + 8;
        } else {
...
void EditScreen::paint (juce::Graphics& g) {
    ...
    text ("Plexi '68 Crunch", displayFont (26.0f), col::ink,
          h.removeFromTop (34), juce::Justification::topLeft);
    drawPill (g, doneRect_.toFloat(), col::accentA (0.0f), col::accentA (0.5f));
    text ("DONE", uiFontTracked (12.0f, true), col::accentAlt, doneRect_, juce::Justification::centred);
    ...
    for (auto& s : stages_) {
        ...
        const auto border = isCab ? col::accentA (0.4f)
                          : s.open ? col::inkA (0.28f) : col::inkA (0.1f);
        ...
        text (s.open ? kChevD : kChevR,
              uiFont (12.0f, false), col::inkA (0.4f),
              hr.removeFromRight (16), juce::Justification::centredRight);
        if (! s.open)
            text (s.summary(), uiFont (13.0f, false), col::inkA (0.5f), hr, juce::Justification::centredLeft);

        if (s.open) {
            for (auto& f : s.faders) {
                auto sb = f.slider->getBounds();
                text (f.name, uiFont (12.0f, false), col::inkA (0.6f),
                      { list_.getX() + 12, sb.getY(), 64, sb.getHeight() }, juce::Justification::centredLeft);
                text (f.fmt (f.slider->getValue()), uiFont (12.0f, true), col::accentAlt,
                      { sb.getRight() + 8, sb.getY(), 40, sb.getHeight() }, juce::Justification::centredRight);
            }
            if (isCab) {
                text ("Greenback 4" + kMult + "12 " + kDot + " IR", uiFont (13.0f, false), col::ink,
                      { list_.getX() + 16, lockRect_.getY(), list_.getWidth() - 140, 32 },
                      juce::Justification::centredLeft);
                drawPill (g, lockRect_.toFloat(), cabLocked_ ? col::accentA (0.12f) : col::accentA (0.0f),
                          col::accentA (0.5f));
                text (cabLocked_ ? kLock + " LOCKED" : juce::String ("UNLOCKED"),
                      uiFontTracked (11.0f, true), col::accentAlt, lockRect_, juce::Justification::centred);
            }
        }
    }
    ...
    text ("smart defaults set " + kDot + " you changed " + juce::String (changed_)
          ...
          juce::Justification::centredLeft);
    text ("RESET", uiFontTracked (12.0f, false), col::inkA (0.55f), resetRect_, juce::Justification::centred);
}

void EditScreen::mouseDown (const juce::MouseEvent& e) {
    const auto p = e.getPosition();
    if (doneRect_.contains (p)) { if (onDone) onDone(); return; }
    if (resetRect_.contains (p)) {
        // Reset every fader to its construction default by re-running defaults.
        static const double defs[][3] = { {28}, {70,50}, {20,75}, {50,44,56}, {0,0}, {25,20} };
        for (int i = 0; i < (int) stages_.size(); ++i)
            for (int j = 0; j < (int) stages_[(size_t) i].faders.size(); ++j)
                stages_[(size_t) i].faders[(size_t) j].slider->setValue (defs[i][j], juce::sendNotification);
        changed_ = 0; repaint(); return;
    }
    for (auto& s : stages_)
        if (s.open && s.lockable && lockRect_.contains (p)) { cabLocked_ = ! cabLocked_; repaint(); return; }
    for (int i = 0; i < (int) stages_.size(); ++i)
        if (stages_[(size_t) i].headerRect.contains (p)) { openOnly (i); return; }
}
