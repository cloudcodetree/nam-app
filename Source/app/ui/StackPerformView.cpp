...
void StackPerformView::layoutGrid () {
    ...
    constexpr int gap = 10, minCellH = 70, maxCellH = 95, cols = 4;
    const int cw = juce::jmax (40, (contentArea_.getWidth () - gap * (cols - 1)) / cols);
    const int rows = (int)((cells_.size () + (size_t)cols - 1) / (size_t)cols);
    // Cell height derives from cell WIDTH, not available height -- a stage
    // switch wants to be roughly square to slightly tall, not stretched
    // into a ~150px pillar just because a light 4-cell grid has a whole
    // screen's worth of room below it. The grid sits directly under the
    // toggle row (no vertical centring); extra scenes simply add rows that
    // grow downward, and any leftover space falls below the grid instead
    // of pushing it away from the toggle (review finding on
    // DONE-03-perform.png: stretched pillar cells + a dead gap above them).
    const int cellH = juce::jlimit (minCellH, maxCellH, cw + 12);
