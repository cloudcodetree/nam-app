...
// Stack detail: header (back chevron / rig name) + body, which is always
// StackEditView (the freeform ordered-chain editor -- the only edit mode
// there is). The gear picker and item sheet overlays are owned here (not by
// StackEditView) so they paint over the header too, per the "overlays cover
// everything, painted last" house rule.
//
// Stripped 2026-08-15 (Chris): the EDIT/CONTROLS tab pill, StackPerformView,
// and everything PERFORM-only (scenes, footswitch assignment, routing) are
// gone -- editing IS the surface now. See decisions.md.
...
public:
    ...
    // freeform rows. See AppShellStackThumbs.cpp.
    ...
    // "+ ADD GEAR" -> the owner computes canAdd-based disabled tabs and
    // opens picker() itself (it also owns the AddChannel/Swap picker modes
    // reached from itemSheet(), so all "what does a picker pick mean"
...
private:
    ...
    juce::Rectangle<int> backRect_, nameRect_, bodyRect_;
