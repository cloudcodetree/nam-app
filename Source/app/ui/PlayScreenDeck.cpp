...
void PlayScreen::paintDeckPanel (juce::Graphics& g) {
    ...
    juce::ColourGradient bg (col::bgGradTop.brighter (0.05f),
                             (float) artRect_.getCentreX(), (float) artRect_.getY(),
                             col::bg, (float) artRect_.getCentreX(),
    ...
    auto thumbOrInitial = [&] (const DeckItem& it, juce::Rectangle<int> rr,
                               float corner, float initialPt) {
        juce::Path tc;
        tc.addRoundedRectangle (rr.toFloat(), corner);
        g.saveState();
        g.reduceClipRegion (tc);
        if (it.thumb.isValid()) {
            g.drawImageWithin (it.thumb, rr.getX(), rr.getY(),
                               rr.getWidth(), rr.getHeight(),
            ...
            g.drawText (it.title.substring (0, 1).toUpperCase(), rr,
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (rr.getBottom() < artRect_.getY() || rr.getY() > artRect_.getBottom())
            continue;
        ...
        if (layoutMode_ == 1) {
            ...
            if (it.kept) {
                const auto hb = in.removeFromRight (22).toFloat()
            ...
            juce::ColourGradient sc (col::bg.withAlpha (0.9f),
                                     (float) rr.getCentreX(), (float) rr.getBottom(),
                                     col::bg.withAlpha (0.0f), (float) rr.getCentreX(),
                                     (float) (rr.getBottom() - scrimH), false);
            ...
            g.drawRoundedRectangle (rr.toFloat().reduced (0.75f), 10.0f,
                                    active ? 1.6f : 1.0f);
        }
    }
    g.restoreState();
