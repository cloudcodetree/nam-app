    ...
    for (size_t i = 0; i < stacks_.size(); ++i) {
        ...
        if ((int) i == selected_) {
            for (int s = 0; s < kNumSlots; ++s) {
                slotRects_[(size_t) s] = { listArea_.getX() + 12, y + 4,
                                           listArea_.getWidth() - 24, slotH - 8 };
                y += slotH;
            }
            y += 6;
        }
...
void StacksScreen::paint (juce::Graphics& g) {
    ...
    text ("Stacks", displayFont (26.0f), col::ink,
          { listArea_.getX(), 8, 220, 44 }, juce::Justification::centredLeft);
    drawPill (g, newBtnRect_.toFloat(), col::accent, col::accent);
    text ("+ NEW STACK", uiFontTracked (10.0f, true), col::inkOnAccent,
          newBtnRect_, juce::Justification::centred);
    ...
    if (stacks_.empty()) {
        text ("no stacks yet " + juce::String::fromUTF8 ("\xC2\xB7")
              ...
              uiFont (13.0f, false), col::inkA (0.45f),
              listArea_, juce::Justification::centred);
    }
    ...
    for (size_t i = 0; i < stacks_.size(); ++i) {
        ...
        if (header.getBottom() >= listArea_.getY() && header.getY() <= listArea_.getBottom()) {
            ...
            text (st.name, uiFont (15.0f, true), col::ink,
                  header.reduced (16, 0).withTrimmedRight (150),
            ...
            text ("LOAD", uiFontTracked (10.0f, true),
                  filled > 0 ? col::accentAlt : col::inkA (0.35f),
                  S (row.applyBtn), juce::Justification::centred);
            text (juce::String::fromUTF8 ("\xC3\x97"), uiFont (16.0f, false),
                  col::inkA (0.4f), S (row.deleteBtn), juce::Justification::centred);
        }
        if (open) {
            for (int s = 0; s < kNumSlots; ++s) {
                ...
                text (slotDefs()[(size_t) s].label, uiFontTracked (8.0f, true),
                      col::inkA (0.4f), in.removeFromTop (13), juce::Justification::centredLeft);
                auto val = in;
                ...
                text (has ? st.titles[(size_t) s] : juce::String ("None"),
                      uiFont (12.0f, true), has ? col::ink : col::inkA (0.4f),
                      val, juce::Justification::centredLeft);
            }
        }
    }
    g.restoreState();
    ...
    if (pickerSlot_ >= 0) {
        ...
        text (juce::String (slotDefs()[(size_t) pickerSlot_].label)
              ...
              { pickerRect_.getX() + 18, pickerRect_.getY() + 10,
                pickerRect_.getWidth() - 36, 16 },
              juce::Justification::centredLeft);
        g.saveState();
        juce::Path clip;
        ...
        if (pickerLoading_) {
            text ("loading" + juce::String::fromUTF8 ("\xE2\x80\xA6"),
                  uiFont (13.0f, false), col::inkA (0.5f),
                  pickerRect_, juce::Justification::centred);
        } else if (pickerTitles_.isEmpty()) {
            text ("nothing found", uiFont (13.0f, false), col::inkA (0.5f),
                  pickerRect_, juce::Justification::centred);
        }
        for (int i = 0; i < pickerTitles_.size(); ++i) {
            const juce::Rectangle<int> row (pickerRect_.getX() + 6,
                pickerRect_.getY() + titleH + pad + i * rowH - (int) pickerScroll_,
                pickerRect_.getWidth() - 12, rowH);
            if (row.getBottom() < pickerRect_.getY() || row.getY() > pickerRect_.getBottom())
                continue;
            text (pickerTitles_[i], uiFont (13.0f, false), col::inkA (0.85f),
                  row.reduced (12, 0), juce::Justification::centredLeft);
        }
        g.restoreState();
        if (pickerContentH_ > pickerRect_.getHeight()) {
            ...
            const float pos = pickerScroll_
                            / (float) juce::jmax (1, pickerContentH_ - pickerRect_.getHeight());
            g.setColour (col::inkA (0.25f));
            g.fillRoundedRectangle ((float) pickerRect_.getRight() - 7.0f,
                                    (float) pickerRect_.getY() + 4.0f + travel * pos,
                                    3.0f, thumbH, 1.5f);
        }
    }
}
...
void StacksScreen::openPicker (int slot) {
    ...
    const int h = juce::jmin (listArea_.getHeight() - 20,
                              getHeight() * 55 / 100);
    pickerRect_ = { getWidth() / 2 - w / 2, listArea_.getY() + 10, w, h };
    pickerContentH_ = 0;
    repaint();
    if (onFetchSlotOptions)
        onFetchSlotOptions (slot,
            [this, slot] (juce::StringArray ids, juce::StringArray titles,
...
void StacksScreen::mouseDown (const juce::MouseEvent& e) {
    ...
    if (newBtnRect_.expanded (4).contains (p)) { if (onCreate) onCreate(); return; }
...
void StacksScreen::mouseDrag (const juce::MouseEvent& e) {
    ...
    if (pickerPressed_) {
        ...
        if (pickerMoved_) {
            pickerScroll_ = juce::jlimit (0.0f,
                (float) juce::jmax (0, pickerContentH_ - pickerRect_.getHeight()),
    ...
    if (listPressed_) {
        ...
        if (listMoved_) {
            scrollY_ = juce::jlimit (0.0f,
                (float) juce::jmax (0, contentH_ - listArea_.getHeight()),
...
void StacksScreen::mouseUp (const juce::MouseEvent& e) {
            ...
            if (i >= 0 && i < pickerTitles_.size() && selected_ >= 0) {
                if (onSlotPicked)
                    onSlotPicked (selected_, pickerSlot_, pickerIds_[i], pickerTitles_[i],
                                  i < pickerFormats_.size() ? pickerFormats_[i]
                                                            : juce::String());
                pickerSlot_ = -1;
                repaint();
            }
    ...
    for (size_t i = 0; i < rows_.size(); ++i) {
        const auto& row = rows_[i];
        if (row.deleteBtn.contains (cp)) { if (onDelete) onDelete ((int) i); return; }
        if (row.applyBtn.contains (cp))  { if (onApply) onApply ((int) i); return; }
        if (row.header.contains (cp))    { if (onSelect) onSelect ((int) i); return; }
    }
    if (selected_ >= 0)
        for (int s = 0; s < kNumSlots; ++s)
