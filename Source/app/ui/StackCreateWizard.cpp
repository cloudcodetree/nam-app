...
// --- Input ------------------------------------------------------------
// Gear mutation (steps 1-3), step-4 FS assignment, and save/template-pick
// live in StackCreateWizardGear.cpp.
...
void StackCreateWizard::mouseDown (const juce::MouseEvent& e) {
    ...
    if (backRect_.expanded (8).contains (p)) return;   // resolved on mouseUp (tap)
    if (!footerRect_.isEmpty () && footerBtnRect_.contains (p)) return;
    if (step_ != Step::Gallery)
        for (int i = 0; i < kStepCount; ++i)
            if (pillRects_[(size_t)i].contains (p)) return;
